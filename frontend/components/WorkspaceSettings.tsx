...
export function WorkspaceSettings({ close }: { close: () => void }) {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('medsync-theme', toggles.darkMode ? 'dark' : 'light');
    window.localStorage.setItem('medsync-settings', JSON.stringify({ toggles, chartDensity, forecastHorizon, accentIntensity }));
